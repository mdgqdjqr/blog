...
<font color=green>解析：</font>由保号性 $\Rightarrow \exist\delta，x\in(-\delta,\delta)，\lim\limits_{x\to0}\frac{f(x)}{|x|}=1>0，\therefore f(x)>0$
